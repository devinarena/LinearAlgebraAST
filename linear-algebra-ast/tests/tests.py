...
# File       : tests.py
# Author     : Devin Arena
# Description: Handles automated test cases for the LinearAlgebraAST.
# Since      : 7/4/2022
########################################################################################
...
ext = "la"
exe_path = os.path.join("..", "target", "debug", "linear-algebra-ast.exe")
...
def run_all_tests():
    passed = 0
    failed = 0
    for case in glob.glob(f"cases/*.{ext}"):
...
def main(argv):
    ...
    if argv[0] == "all":
        ...
        case = argv[0]
        if os.path.exists(os.path.join("cases", case + f".{ext}")):
...
if __name__ == "__main__":
    if len(sys.argv) <= 1:
        print("LinearAlgebraAST automated testing client.")
        print("Developed by Devin Arena.")
        print("Usage: python3 tests.py [all|test_name|compile]")
        print("\tall: Runs all test cases.")
        print(
            f"\tcompile: Compiles all test cases (runs .{ext} files and places their output into cases/)."
        )
